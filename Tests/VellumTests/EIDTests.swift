...
struct EIDTests {
  ...
  @Test func groupCloneTemplateEID_mapsToOtherWithSameName() {
    ...
    #expect(child.clonableGroupTemplateEID == EID.other(name: "PlayerSet__groupclone__Hand"))
  }

  /// Standalone clones and `.other` entities have no template preset key.
  @Test func groupCloneTemplateEID_nilForNonGroupClones() {
    #expect(EID.clone(name: "GoStoneBlack", cloneId: UUID()).clonableGroupTemplateEID == nil)
    #expect(EID.other(name: "PlayerSet__groupclone__Hand").clonableGroupTemplateEID == nil)
  }
  ...
  @Test func clonableGroupChild_roundTripsStringFormat() throws {
    ...
    #expect(decoded.isClonableGroupChild)
    #expect(!decoded.isPureClone)
    #expect(decoded.clonableGroupName == "PlayerSet")
  ...
  @Test func pureCloneAndGroupChild_areDistinctFlavors() {
    ...
    #expect(pure != group)
    #expect(pure.isPureClone && !pure.isClonableGroupChild)
    #expect(group.isClonableGroupChild && !group.isPureClone)
    #expect(pure.clonableGroupName == nil)
  }
}
